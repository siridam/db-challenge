...
--------------------------------------------------
-- Create Function USER_SCHEMA.F_LOGOUT
---------------------------------------------------

drop function if exists USER_SCHEMA.F_LOGOUT;
CREATE OR REPLACE FUNCTION USER_SCHEMA.F_LOGOUT (
     LOGIN VARCHAR
)
    RETURNS VARCHAR
LANGUAGE PLPGSQL
    ...
    DECLARE STATUS_CODE VARCHAR;
BEGIN
IF EXISTS ( SELECT 1 FROM USER_SCHEMA.LOG_HISTORY WHERE LOGIN_ID = LOGIN 
              AND STATUS = 'SUCCESS' AND SESSION_END_TIME IS NULL)
THEN 
    UPDATE USER_SCHEMA.LOG_HISTORY
...
GRANT EXECUTE ON FUNCTION USER_SCHEMA.F_LOGOUT TO MASTER_RO;
GRANT EXECUTE ON FUNCTION USER_SCHEMA.F_LOGOUT TO MASTER_RW;
